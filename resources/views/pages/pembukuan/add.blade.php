...
@section("content")
    ...
    <main class="bg-bg2 h-[calc(100vh-4rem)] flex justify-center items-center">
        <form action="{{url("/add")}}" method="post" class="flex flex-col bg-white gap-4 pb-4 rounded-xl w-1/3">
            <h1 class="text-center font-semibold text-xl bg-primary p-4 text-white rounded-xl">Tambah data Keuangan</h1>
            @csrf
            <div class="px-8 flex flex-col gap-4">
                ...
                <input type="number" step=".0001" name="pemasukan" id="pemasukan"
                       placeholder="Pemasukan Keuangan" class="w-full border rounded-md p-2">
                <label for="pengeluaran">Pengeluaran</label>
                <input type="number" step=".0001" name="pengeluaran" id="pengeluaran"
