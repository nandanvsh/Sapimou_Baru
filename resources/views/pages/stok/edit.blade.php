...
@section('content')
    ...
    <main class="min-h-[calc(100vh-4rem)] bg-bg3 p-8 flex justify-center">
        <form action="{{url("/editstok/$stok->id")}}" method="post" class=" w-1/3 flex flex-col bg-white gap-4 pb-4 rounded-xl">
            <h1 class="text-center font-semibold text-xl bg-primary text-white p-4 rounded-xl">Mengubah data stok</h1>
            @csrf
            <div class="px-8 flex flex-col gap-4">
                ...
                <textarea name="description" id="description" cols="30" rows="8"
                          class="resize-none w-full border rounded-md p-2"
                          placeholder="Deskripsi...">{{$stok->description}}</textarea>
                <div class="flex justify-center gap-4">
                    <a href="{{url("/stok")}}" class="bg-red-600 text-white p-2 rounded-full flex-grow text-center">Batal</a>
                    <button type="submit" class="bg-btn-blue text-white p-2 rounded-full flex-grow text-center">Simpan
                    </button>
                </div>
            </div>
        </form>
